...
2.  Make a scatterplot of `hwy` vs. `displ` using the `mpg` data frame. Next, map a third, numerical variable to `color`, then `size`, then both `color` and `size`, then `shape`. How do these aesthetics behave differently for categorical vs. numerical variables?
    ...
    -   numerical gives a color **gradient**, while categorical gives **distinct** colors
...
3.  In the scatterplot of `hwy` vs. `displ`, what happens if you map a third variable to `linewidth`?
    -   in `geom_point` there's no `linewidth`
...
4.  What happens if you map the same variable to multiple aesthetics? they all map to that variable

5.  Make a scatterplot of `bill_depth_mm` vs. `bill_length_mm` and color the points by `species`. What does adding coloring by species reveal about the relationship between these two variables? *I can clearly see three clusters.* What about faceting by `species`? *it makes it even clearer*
